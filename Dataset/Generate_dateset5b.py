...
output_dir_img = "/home/felix/SPRI_AI_Project/Dataset/Crops/True"
output_dir_mask = "/home/felix/SPRI_AI_Project/Dataset/Crops/Mask"
...
def dividir_y_filtrar_imagen(ruta_imagen_true, ruta_imagen_mask, tamaño_bloque=TAMANO_BLOQUE, 
    ...
    try:
        ...
        print(f"    Leyendo imagen de 5 bandas...")
        with rasterio.open(ruta_imagen_true) as src_true:
            if src_true.count < 4:
                print(f"    ❌ Imagen tiene {src_true.count} bandas, se esperaban 5")
                return 0
            
            # Leer todas las bandas de una vez
            bandas_true = [src_true.read(i) for i in range(1, 6)]
...
def procesar_zip(zip_path, estadisticas_globales):
    ...
    try:
        ...
        compute_ndvi(
            nir_scaled_path=scaled_files[3],  # NIR es la última banda escalada
            red_scaled_path=scaled_files[0],  # Red es la primera banda escalada
