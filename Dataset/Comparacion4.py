...
def procesar_prediccion(imagen_prediccion):
    ...
    1. Detecta incendios basados en colores rojos Y BLANCOS
    ...
    if imagen_prediccion.shape[2] >= 3:
        ...
        print(f"  Analizando imagen de predicción...")
        print(f"  Valores R: min={r.min()}, max={r.max()}, mean={r.mean():.1f}")
        print(f"  Valores G: min={g.min()}, max={g.max()}, mean={g.mean():.1f}")
        print(f"  Valores B: min={b.min()}, max={b.max()}, mean={b.mean():.1f}")
        
        # MÁSCARA 1: Detectar píxeles ROJOS (incendios en algunos modelos)
        # R alto, G y B bajos
        mascara_rojos = (r > 200) & (g < 50) & (b < 50)
        rojos_count = mascara_rojos.sum()
        print(f"  Píxeles ROJOS detectados: {rojos_count}")
        
        # MÁSCARA 2: Detectar píxeles BLANCOS (incendios en otros modelos)
        # R, G, B altos
        mascara_blancos = (r > 200) & (g > 200) & (b > 200)
        blancos_count = mascara_blancos.sum()
        print(f"  Píxeles BLANCOS detectados: {blancos_count}")
        
        # MÁSCARA 3: Detectar píxeles AMARILLOS/ANARANJADOS (otros posibles incendios)
        # R alto, G medio-alto, B bajo
        mascara_amarillos = (r > 180) & (g > 100) & (b < 100)
        amarillos_count = mascara_amarillos.sum()
        print(f"  Píxeles AMARILLOS detectados: {amarillos_count}")
        
        # MÁSCARA 4: Detectar cualquier color NO NEGRO (para imágenes binarias)
        # Si alguno de los canales es significativo
        mascara_no_negro = (r > 50) | (g > 50) | (b > 50)
        no_negro_count = mascara_no_negro.sum()
        print(f"  Píxeles NO NEGROS detectados: {no_negro_count}")
        
        # COMBINAR todas las máscaras: Cualquier píxel que sea rojo, blanco, amarillo o no negro
        mascara_incendios = mascara_rojos | mascara_blancos | mascara_amarillos | mascara_no_negro
        
        print(f"  TOTAL incendios detectados en predicción: {mascara_incendios.sum()}")
...
def comparar_imagenes(ruta_original, ruta_prediccion, umbral_rojo=200):
    ...
    print("\nProcesando imagen de predicción con fondo blanco...")
...
def visualizar_tres_imagenes(imagen_original, imagen_prediccion, imagen_diferencias, estadisticas, titulo="Comparación de Imágenes"):
    ...
    leyenda_texto = (
        ...
        f"DETECCIÓN EN PREDICCIÓN:\n"
        f"• Píxeles ROJOS (R>200, G<50, B<50)\n"
        f"• Píxeles BLANCOS (R>200, G>200, B>200)\n"
        f"• Píxeles AMARILLOS (R>180, G>100, B<100)\n"
        f"• Cualquier píxel NO NEGRO\n\n"
        f"{'='*20}\n"
        f"RESUMEN ESTADÍSTICO:\n\n"
        f"Precisión: {estadisticas['precision']*100:.2f}%\n"
        f"Recall: {estadisticas['recall']*100:.2f}%\n"
...
def guardar_resultados(ruta_salida, imagen_diferencias, estadisticas, geotransform=None, projection=None):
    ...
    with open(ruta_estadisticas, 'w') as f:
        ...
        f.write("• Imagen predicción: Se extrajeron múltiples tipos de píxeles:\n")
        f.write("   - Píxeles ROJOS (R>200, G<50, B<50)\n")
        f.write("   - Píxeles BLANCOS (R>200, G>200, B>200)\n")
        f.write("   - Píxeles AMARILLOS (R>180, G>100, B<100)\n")
        f.write("   - Cualquier píxel NO NEGRO (R>50 o G>50 o B>50)\n")
...
def main():
    ...
    print("Original: Detecta solo ROJOS")
    print("Predicción: Detecta ROJOS, BLANCOS, AMARILLOS y NO NEGROS")
    print("=" * 70)
