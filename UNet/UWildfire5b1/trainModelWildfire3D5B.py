...
DATA_PATH = '/home/liese2/SPRI_AI_project/UNet/UWildfire5b1/data'
...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Buscar imagen con extensión (igual que con máscaras)
        base_name = os.path.splitext(img_name)[0]  # por si ya trae extensión
        img_path = None
        for ext in ['.tif', '.tiff', '.TIF', '.TIFF']:
            ...
            if os.path.exists(potential_path):
                ...
                break
        # También probar el nombre tal cual (por si ya tiene extensión)
        if img_path is None:
            raw_path = os.path.join(self.images_dir, img_name)
            if os.path.exists(raw_path):
                img_path = raw_path

        if img_path is None:
            print(f"ERROR: No se encontró imagen para {img_name}")
            return torch.zeros((5, self.target_size, self.target_size)), \
    ...
    def load_and_resize_image(self, path, target_size):
        ...
        if bands != 5:
            print(f"ADVERTENCIA: La imagen {path} tiene {bands} bandas, se esperaban 5")
        
        height, width = ds.RasterYSize, ds.RasterXSize
        
        if height == target_size and width == target_size:
            image = np.zeros((min(bands, 5), target_size, target_size), dtype=np.float32)
